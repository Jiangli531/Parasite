...
def train(helper, epoch, poison_lr, train_data_sets, local_model, target_model, contrastive_model, is_poison, last_weight_accumulator=None, mask_grad_list=None):
    ...
    for model_id in range(helper.params['no_models']):
        ...
        if epoch <= 500:
            ...
            lr = epoch*(-target_lr)/1500 + target_lr*4.0/3.0 
            if lr <= 0.0001: 
                lr = 0.0001

        if epoch > helper.params['poison_epochs'][-1]: 
            lr = helper.params['persistence_diff']
            ...
            try:
                for internal_epoch in range(1, retrain_no_times + 1):
                    ...
                    if helper.params['type'] == 'text':
                        ...
                        if helper.params['dataset'] == 'cifar10' or helper.params['dataset'] == 'emnist':
                            _, data_iterator = helper.train_data[0]
                        elif helper.params['dataset'] == 'gtsrb' or helper.params['dataset'] == 'BelgiumTS':
...
def test_poison(helper, epoch, data_source,
    ...
    if helper.params['type'] == 'text':
        ...
        dataset_size = len(data_source.dataset) - helper.remove_label
...
def train_generator(helper, generator_lr, train_data, output_name):
    ...
    for _, (inputs, labels) in enumerate(helper.nine_loader):
        ...
        if inputs.shape[0] != 64:
            continue
        feature = helper.target_model.feature_forward(inputs)
    ...
    for epoch in range(helper.params['generator_epoch']):
        for i, (inputs, labels) in enumerate(train_data):
            ...
            out = helper.generator(inputs)
            trigger = out[0]
            mu = out[2]
            log_var = out[3]
            
            poison_images = helper.generator.add_trigger(inputs, trigger)
            poison_feature = helper.target_model.feature_forward(poison_images)
           
            distance = torch.nn.functional.cosine_similarity(poison_feature, clean_feature)
            l_f = torch.mean(distance)

            trigger_temp = trigger.view(trigger.shape[0], helper.params['trigger_size']*helper.params['trigger_size'])/255
            
            l_d = torch.mean(torch.sum((trigger_temp.unsqueeze(dim=1) - trigger_temp.unsqueeze(dim=0)) ** 2, dim=2))
            ...
            loss_generator = recons_loss + 10*kld_loss

            loss_generator.backward()
            optimizer_generator.step()
            if i % 100 == 0:
                logger.info('Iteration {}, Generator loss: {:.4f}, l_f: {:.4f}, l_d: {:.4f}, recons_loss: {:.4f}, '
                      'kld_loss: {:.4f}'.format(i, loss_generator.item(), l_f.item(), l_d.item(), recons_loss.item(), kld_loss.item()))
        logger.info('Epoch {}, Generator loss: {:.4f}, l_f: {:.4f}, l_d: {:.4f}, recons_loss: {:.4f}, kld_loss: {:.4f}'
              .format(epoch, loss_generator.item(), l_f.item(), l_d.item(), recons_loss.item(), kld_loss.item()))
        
        torch.save(helper.generator.state_dict(), './saved_models/'+output_name+'/generator_'+str(epoch)+'.pth')
...
if __name__ == '__main__':
    ...
    if helper.params['generator_epoch'] != 0:
        train_generator(helper=helper, generator_lr=helper.params['generator_lr'], train_data=data_iterator_generator,
                    output_name=helper.params['output_name'])
    
    if helper.params['dataset'] == 'cifar10':
        generator_path = './saved_models/vae-train/generator_alpha_0.5_epoch_7.pth'
    elif helper.params['dataset'] == 'emnist':
        generator_path = './saved_models/emnist-vae-train-log-normalized-unitmask-1/generator_alpha_0.75_epoch_2.pth'
    elif helper.params['dataset'] == 'gtsrb':
        generator_path = './saved_models/gtsrb/generator_alpha_0.5_epoch_2.pth'
    else:
        generator_path = './saved_models/BelgiumTS-vae-train-log-normalized-minmuml_d/generator_alpha_0.5_epoch_9.pth'
    logger.info(f'The geneator is load from: {generator_path}')
    helper.generator.load_state_dict(torch.load(generator_path))
    helper.generator.eval()
    ...
    for epoch in range(helper.start_epoch, helper.params['epochs'] + 1):
        ...
        if helper.params['is_poison']:
            ...
            csv_record.posiontest_result.append(
                [epoch, epoch_loss_p, float(epoch_acc_p)])
        ...
        if helper.params['test_forgetting']: 
            ...
            epoch_loss, epoch_acc = test(helper=helper, epoch=epoch, data_source=helper.test_data,
                                    model=helper.target_model, is_poison=False, visualize=True)
        csv_record.test_result.append([epoch, epoch_loss, epoch_acc])
